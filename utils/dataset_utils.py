...
class PromptTrainDataset(Dataset):
    ...
    def _init_snow_ids(self):
        temp_ids = []
        ss = self.args.data_file_dir + "snowy/snowTrain.txt"
        temp_ids += [self.args.desnow_dir + id_.strip() for id_ in open(ss)]
        # build list of {"clean_id": ..., "de_type":6}
        self.ss_ids = [{"clean_id": x, "de_type": 6} for x in temp_ids]

        self.snow_counter = 0
        self.num_snow = len(self.ss_ids)
    ...
    def _get_rain_gt(self, rainy_path):
        # rainy_path: …/Derain/rainy/rain-1.png
        base = os.path.dirname(os.path.dirname(rainy_path))
        fname = os.path.basename(rainy_path)
        idx = fname.split("rain-")[-1]
        return os.path.join(
            base, "gt", f"rain_clean-{idx}"
        )  # …/Derain/gt/rain_clean-1.png

    def _get_snow_gt(self, snowy_path):
        # snowy_path: …/Desnow/snowy/snow-1.png
        base = os.path.dirname(os.path.dirname(snowy_path))
        fname = os.path.basename(snowy_path)
        idx = fname.split("snow-")[-1]
        return os.path.join(base, "gt", f"snow_clean-{idx}")
    ...
    def __getitem__(self, idx):
        ...
        if de_id < 3:
            ...
            if de_id == 3:
                ...
                clean_name = self._get_rain_gt(sample["clean_id"])
                clean_img = crop_img(
                ...
                clean_name = self._get_snow_gt(sample["clean_id"])
                clean_img = crop_img(
                    np.array(Image.open(clean_name).convert("RGB")), base=16
                )
...
class DenoiseTestDataset(Dataset):
    ...
    def _add_gaussian_noise(self, clean_patch):
        ...
        noisy_patch = np.clip(clean_patch + noise * self.sigma, 0, 255).astype(
            np.uint8
        )
    ...
    def __getitem__(self, clean_id):
        ...
        clean_img, noisy_img = self.toTensor(clean_img), self.toTensor(
            noisy_img
        )
    ...
    def tile_degrad(input_, tile=128, tile_overlap=0):
        ...
        for h_idx in h_idx_list:
            for w_idx in w_idx_list:
                ...
                E[..., h_idx: (h_idx + tile), w_idx: (w_idx + tile)].add_(
                    out_patch
                )
                W[..., h_idx: (h_idx + tile), w_idx: (w_idx + tile)].add_(
                    out_patch_mask
                )
        # restored = E.div_(W)
...
class DerainDehazeDataset(Dataset):
    ...
    def _init_input_ids(self):
        if self.task_idx == 0:
            ...
            self.ids += [
                self.args.derain_path + "input/" + id_ for id_ in name_list
            ]
        elif self.task_idx == 1:
            self.ids = []
            name_list = os.listdir(self.args.dehaze_path + "input/")
            self.ids += [
                self.args.dehaze_path + "input/" + id_ for id_ in name_list
            ]
    ...
    def __getitem__(self, idx):
        ...
        clean_img, degraded_img = self.toTensor(clean_img), self.toTensor(
            degraded_img
        )
